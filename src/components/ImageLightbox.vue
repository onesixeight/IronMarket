<template>
  <teleport to="body">
    <transition name="lightbox">
      <div v-if="visible" class="fixed inset-0 z-[80] flex items-center justify-center" @click.self="$emit('close')">
        <div class="absolute inset-0 bg-iron-900/90 backdrop-blur-md"></div>
        <div class="relative z-10 max-w-5xl max-h-[90vh] p-4">
          <button @click="$emit('close')" class="absolute -top-1 -right-1 p-2 bg-white/10 hover:bg-white/20 rounded-xl text-cream-200 transition-colors">
