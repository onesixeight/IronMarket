...
assert.match(indexHtml, /<link rel="canonical" href="https:\/\/etalon-kovka\.vercel\.app\/" \/>/)
...
assert.match(sitemap, /<loc>https:\/\/etalon-kovka\.vercel\.app\/<\/loc>/)
assert.match(sitemap, /<loc>https:\/\/etalon-kovka\.vercel\.app\/catalog<\/loc>/)
assert.match(sitemap, /<loc>https:\/\/etalon-kovka\.vercel\.app\/contacts<\/loc>/)
assert.match(sitemap, /<loc>https:\/\/etalon-kovka\.vercel\.app\/delivery<\/loc>/)
assert.match(sitemap, new RegExp(`<loc>https://etalon-kovka\\.vercel\\.app/catalog/${catalog.categories[0].slug}</loc>`))
assert.match(sitemap, new RegExp(`<loc>https://etalon-kovka\\.vercel\\.app/product/${catalog.products[0].id}</loc>`))
...
assert.match(robots, /Sitemap: https:\/\/etalon-kovka\.vercel\.app\/sitemap\.xml/)
